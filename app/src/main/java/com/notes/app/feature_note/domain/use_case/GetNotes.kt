...
class GetNotes(private val repository: NoteRepository) {
    operator fun invoke(
        noteOrder: NoteOrder = NoteOrder.DateCreated(OrderType.Ascending)
    ): Flow<List<Note>> {
            ...
            when (noteOrder.orderType) {
                is OrderType.Ascending -> {
                    when (noteOrder) {
                        ...
                        is NoteOrder.DateModified -> notes.sortedBy { it.dateModified }
                    }
                }
                ...
                is OrderType.Descending -> {
                    when (noteOrder) {
                        ...
                        is NoteOrder.DateModified -> notes.sortedByDescending { it.dateModified }
